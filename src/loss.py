...
class MatroshkaTwinsLoss(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        on_diag = torch.sum(
            ((1 - torch.diagonal(c)) ** 2) * torch.diagonal(self.term_weights)
        )
        off_diag = torch.sum(
...
class PositionWeightedBarlowTwins(nn.Module):
    """Inspired by matroshka embeddings but with a different weighting scheme."""

    def __init__(self, max_size: int, exp: float = 1, lambda_: float = 1.0):
        super().__init__()
        self.lambda_ = lambda_
        self.term_weights = self._calculate_weights(exp, max_size)
